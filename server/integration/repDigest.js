...
async function sendDigestFor(rep) {
  ...
  const yesterdayOrders = await dbx.prepare(`
  ...
  const draft = await buildRepDigestEmail(rep, { yesterdayOrders, today: daySummary });
...
export async function sendAllRepDigests(trigger = 'schedule') {
  ...
  await setSetting('last_rep_digest_at', new Date().toISOString());
  await setSetting('last_rep_digest_result', results.map((r) => `${r.rep}:${r.status}`).join(' · '));
...
async function dueNow() {
  if (await getSetting('rep_digest_enabled', '0') !== '1') return false;
  const last = await getSetting('last_rep_digest_at', null);
  const minsSince = last ? (Date.now() - new Date(last).getTime()) / 60000 : Infinity;
  const [h, m] = (await getSetting('rep_digest_time', '07:00')).split(':').map(Number);
  const now = new Date();
...
export function startRepDigestScheduler() {
  setInterval(async () => {
    try {
      if (await dueNow()) await sendAllRepDigests('scheduled');
    } catch (e) {
