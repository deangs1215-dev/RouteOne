...
router.get('/drafts/:id', (req, res) => {
  const row = db.prepare('SELECT * FROM drafts WHERE id = ? AND rep_id = ?').get(req.params.id, req.user.id);
...
router.put('/drafts/:id', (req, res) => {
  const existing = db.prepare('SELECT id FROM drafts WHERE id = ? AND rep_id = ?').get(req.params.id, req.user.id);
  if (!existing) return res.status(404).json({ error: 'Draft not found' });
  const { label, data } = req.body;
  db.prepare(`UPDATE drafts SET label = COALESCE(?, label), data = ?, updated_at = datetime('now') WHERE id = ?`)
    .run(label || null, JSON.stringify(data || {}), req.params.id);
  res.json({ ok: true });
});

router.delete('/drafts/:id', (req, res) => {
  const result = db.prepare('DELETE FROM drafts WHERE id = ? AND rep_id = ?').run(req.params.id, req.user.id);
