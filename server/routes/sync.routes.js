...
router.get('/sync/snapshot', async (req, res) => {
  ...
  const products = (await dbx.prepare(`
  ...
  const contractPrices = await dbx.prepare(`
  ...
  const visitsToday = await dbx.prepare(`
  ...
  const formTemplates = (await dbx.prepare('SELECT * FROM form_templates WHERE active = 1')
