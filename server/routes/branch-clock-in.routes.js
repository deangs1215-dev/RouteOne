...
router.post('/branch-clock-in', requireAuth, (req, res) => {
  ...
  try {
    ...
    const clockIn = db.prepare('SELECT * FROM branch_clock_ins WHERE id = ?').get(result.lastInsertRowid);
    res.json(clockIn);
  } catch (e) {
...
router.post('/branch-clock-in/:id/clock-out', requireAuth, (req, res) => {
  try {
    const clockIn = db.prepare('SELECT * FROM branch_clock_ins WHERE id = ? AND rep_id = ?').get(req.params.id, req.user.id);
    ...
    const updated = db.prepare('SELECT * FROM branch_clock_ins WHERE id = ?').get(req.params.id);
    res.json(updated);
  } catch (e) {
...
router.get('/branch-clock-in/today', requireAuth, (req, res) => {
  try {
...
router.put('/branch-clock-in/:id', requireAuth, (req, res) => {
  const { notes } = req.body;
  try {
    const clockIn = db.prepare('SELECT * FROM branch_clock_ins WHERE id = ? AND rep_id = ?').get(req.params.id, req.user.id);
    if (!clockIn) return res.status(404).json({ error: 'Clock in not found' });
