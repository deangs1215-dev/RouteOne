...
router.post('/route-cycles/import', requireRole('admin', 'manager'), (req, res) => {
  ...
  const findCustomer = db.prepare('SELECT id FROM customers WHERE code = ?');
  ...
  for (const wk of b.weeks) {
    ...
    for (const [dayName, codes] of Object.entries(wk.days || {})) {
      ...
      codes.forEach((raw, i) => {
        ...
        if (!code) return;
        const cust = findCustomer.get(code);
  ...
  const save = db.transaction(() => {
    ...
    db.prepare('UPDATE route_cycles SET active = 0 WHERE rep_id = ? AND active = 1').run(b.rep_id);
    ...
    const insert = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `);
    for (const r of rows) insert.run(cycleId, r.weekNo, r.weekday, r.customer_id, r.code, r.seq);
    return cycleId;
  });
  const cycleId = save();
  logActivity(req.user.id, 'import', 'route_cycle', cycleId, { rep_id: b.rep_id, stops: rows.length });
...
router.get('/route-cycles', requireRole('admin', 'manager', 'office'), (req, res) => {
...
router.get('/route-cycles/:id', requireRole('admin', 'manager', 'office'), (req, res) => {
  const cycle = db.prepare('SELECT rc.*, u.name AS rep_name FROM route_cycles rc JOIN users u ON u.id = rc.rep_id WHERE rc.id = ?').get(req.params.id);
  if (!cycle) return res.status(404).json({ error: 'Cycle not found' });
  cycle.stops = db.prepare(`
...
router.delete('/route-cycles/:id', requireRole('admin', 'manager'), (req, res) => {
  db.prepare('DELETE FROM route_cycle_stops WHERE cycle_id = ?').run(req.params.id);
  db.prepare('DELETE FROM route_cycles WHERE id = ?').run(req.params.id);
  res.json({ ok: true });
});

// --- Compliance ---------------------------------------------------------------
// Of the customers planned in [from,to], how many were visited that same week.
router.get('/route-compliance', requireRole('admin', 'manager', 'office'), (req, res) => {
  ...
  const visitsInRange = db.prepare(`
  ...
  while (cursor <= lastMonday) {
    ...
    if (wSince >= 0) {
      ...
      if (planned && planned.size) {
        ...
        const visitedRows = visitsInRange.all(repId, weekStart, weekEnd);
  ...
  if (missed.length) {
    const names = db.prepare(`SELECT id, name, city FROM customers WHERE id IN (${missed.map(() => '?').join(',')})`)
...
router.get('/reps/:id/summary', requireRole('admin', 'manager', 'office'), (req, res) => {
  ...
  `).get(id, month);
  rep.customers_assigned = db.prepare("SELECT COUNT(*) AS n FROM customers WHERE rep_id = ? AND status = 'active'").get(id).n;
  rep.recent_orders = db.prepare(`
