...
router.post('/visits', async (req, res) => {
  ...
  await logActivity(req.user.id, 'plan', 'visit', info.lastInsertRowid);
...
router.get('/visits/open', async (req, res) => {
  const v = await dbx.prepare(`
...
router.post('/visits/check-in', async (req, res) => {
  ...
  const visit = await dbx.prepare('SELECT * FROM visits WHERE id = ?').get(visitId);
  ...
  await logActivity(req.user.id, 'check_in', 'visit', visitId, { check_in_type: checkInType, ...(dist != null ? { distance_m: dist } : {}) });
  res.json(await dbx.prepare('SELECT * FROM visits WHERE id = ?').get(visitId));
});

router.post('/visits/:id/check-out', async (req, res) => {
  const b = req.body || {};
  const visit = await dbx.prepare('SELECT * FROM visits WHERE id = ?').get(req.params.id);
  if (!visit) return res.status(404).json({ error: 'Visit not found' });
  if (scopeForUser(req.user).isRep && visit.rep_id !== req.user.id) {
    return res.status(403).json({ error: 'Not your visit' });
  }
  ...
  await logActivity(req.user.id, 'check_out', 'visit', req.params.id, { outcome: b.outcome });
  res.json(await dbx.prepare('SELECT * FROM visits WHERE id = ?').get(req.params.id));
});
...
router.post('/visits/log-offline', async (req, res) => {
  const b = req.body || {};
  if (!b.customer_id) return res.status(400).json({ error: 'Customer is required' });
  if (!await dbx.prepare('SELECT 1 FROM customers WHERE id = ?').get(b.customer_id)) {
    return res.status(404).json({ error: 'Customer not found' });
  ...
  await logActivity(req.user.id, 'log_offline', 'visit', info.lastInsertRowid);
  res.json(await dbx.prepare('SELECT * FROM visits WHERE id = ?').get(info.lastInsertRowid));
});
...
router.post('/visits/:id/photos', async (req, res) => {
  const visit = await dbx.prepare('SELECT * FROM visits WHERE id = ?').get(req.params.id);
  if (!visit) return res.status(404).json({ error: 'Visit not found' });
  if (scopeForUser(req.user).isRep && visit.rep_id !== req.user.id) {
    return res.status(403).json({ error: 'Not your visit' });
  }
  const count = (await dbx.prepare('SELECT COUNT(*) AS n FROM visit_photos WHERE visit_id = ?').get(visit.id)).n;
  ...
  const info = await dbx.prepare('INSERT INTO visit_photos (visit_id, path, caption) VALUES (?, ?, ?)')
    .run(visit.id, path, req.body?.caption || null);
  await logActivity(req.user.id, 'photo', 'visit', visit.id);
  res.json(await dbx.prepare('SELECT * FROM visit_photos WHERE id = ?').get(info.lastInsertRowid));
});

router.get('/visits/:id/photos', async (req, res) => {
  if (scopeForUser(req.user).isRep) {
    const visit = await dbx.prepare('SELECT rep_id FROM visits WHERE id = ?').get(req.params.id);
    if (!visit || visit.rep_id !== req.user.id) return res.status(403).json({ error: 'Not your visit' });
  }
  res.json(await dbx.prepare('SELECT * FROM visit_photos WHERE visit_id = ? ORDER BY created_at').all(req.params.id));
});

// Delete a photo from a visit.
router.delete('/visits/:id/photos/:photoId', async (req, res) => {
  const photo = await dbx.prepare('SELECT * FROM visit_photos WHERE id = ? AND visit_id = ?').get(req.params.photoId, req.params.id);
  if (!photo) return res.status(404).json({ error: 'Photo not found' });
  const visit = await dbx.prepare('SELECT rep_id FROM visits WHERE id = ?').get(photo.visit_id);
  if (scopeForUser(req.user).isRep && visit.rep_id !== req.user.id) {
    return res.status(403).json({ error: 'Not your visit' });
  }
  await dbx.prepare('DELETE FROM visit_photos WHERE id = ?').run(photo.id);
  deleteUploadedFile(photo.path);
  await logActivity(req.user.id, 'photo_delete', 'visit', photo.visit_id);
  res.json({ ok: true });
});

// Visit summary: orders, quotes, forms, and photos created during this visit
router.get('/visits/:id/summary', async (req, res) => {
  const visitId = req.params.id;
  const visit = await dbx.prepare(`
  ...
  const photos = await dbx.prepare(`
...
export async function buildDaySummary(repId, date) {
  const visits = await dbx.prepare(`
    ...
    ORDER BY CASE WHEN v.status = 'completed' THEN 1 ELSE 0 END, CASE WHEN v.route_order IS NULL THEN 1 ELSE 0 END, v.route_order, v.id
  ...
  stats.target = await repMonthTarget(repId, Number(date.slice(5, 7)));
  return { visits, stats };
}
...
router.put('/visits/:id', async (req, res) => {
  ...
  await logActivity(req.user.id, 'edit', 'visit', req.params.id);
  res.json(await dbx.prepare('SELECT * FROM visits WHERE id = ?').get(req.params.id));
});
