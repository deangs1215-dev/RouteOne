...
export default function Visits() {
  ...
  useEffect(() => {
    // Reps only ever see their own visits (enforced server-side regardless of
    // this list), so there's no point fetching/showing every rep's name to them.
    if (isRep) return;
          ...
          <Table headers={isRep ? ['Customer', 'Check-in', 'Check-out', 'Status', 'Outcome', 'Orders'] : ['Customer', 'Rep', 'Check-in', 'Check-out', 'Status', 'Outcome', 'Orders']}
            ...
            {rows.map((v) => (
              <tr key={v.id} className="hover:bg-slate-50">
                ...
                {!isRep && <td className="td text-slate-500">{v.rep_name}</td>}
